...
import { Facility, getPerformanceVariant } from '@/features/facilities/services/facility.service'
      ...
      <table className="text-sm bg-gray-900/60 backdrop-blur-md border border-border" style={{ minWidth: '1400px', borderRadius: '0.5rem' }}>
        <thead>
          <tr className="border-b border-border bg-background/60">
            ...
            <th className="text-left px-4 py-3 font-medium text-muted-foreground min-w-[140px]">Performance</th>
        ...
        <tbody>
          {facilities.map((facility) => {
            const performance = facility.performance ?? 0
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge 
                      variant="outline"
                      className={`text-xs ${
                        performance >= 80 ? 'bg-green-500/10 text-green-500 border-green-500/30' :
                        performance >= 60 ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30' :
                        performance >= 40 ? 'bg-orange-500/10 text-orange-500 border-orange-500/30' :
                        'bg-red-500/10 text-red-500 border-red-500/30'
                      }`}
                    >
                      {getPerformanceVariant(performance).replace('performance_', '').toUpperCase()}
                    </Badge>
                  </div>
                </td>
